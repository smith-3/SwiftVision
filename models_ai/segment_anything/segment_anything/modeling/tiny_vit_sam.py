...
# Suponiendo que tienes un registro de modelos global
_model_registry = {}
...
def register_tiny_vit_model(fn):
    ...
    def fn_wrapper(pretrained=False, **kwargs):
        model = fn(pretrained=pretrained, **kwargs)  # Asegúrate de pasar kwargs al modelo
        if pretrained:
    ...
    # renombrar el nombre de fn_wrapper
    fn_wrapper.__name__ = fn.__name__

    # Comprobar si el modelo ya está registrado
    model_name = fn.__name__
    if model_name not in _model_registry:
        _model_registry[model_name] = fn_wrapper
        # print(f"Model {model_name} registered.")
    else:
        print(f"Warning: Model {model_name} already registered, skipping registration.")
    
    return fn_wrapper  # Devuelve el envoltorio
