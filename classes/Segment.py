...
class ImageSegmentation:
    def __init__(self, checkpoint_path: str = "./pretrained_models/sam_vit_h_4b8939.pth"):
    ...
    def setup_model(self, model_type = "vit_h"):
    ...
    def hair_color(self, mask, color, output_path: str):
        """
        Change the hair color of the selected mask to the specified color and save the image.
        
        Args:
            mask (np.ndarray): The segmentation mask as a numpy array.
            color (tuple): The color to apply (R, G, B).
        ...
        if not isinstance(color, tuple) or len(color) != 3:
            raise ValueError("Color must be a tuple with 3 elements (R, G, B).")

        # Smooth the edges of the segmentation mask
        blurred_mask = cv2.GaussianBlur(mask.astype(np.float32), (21, 21), sigmaX=10, sigmaY=10)

        # Convert the input image to LAB color space
        image_lab = cv2.cvtColor(self.image, cv2.COLOR_RGB2LAB)
        ...
        # Determine the lighten or darken adjustment
        if not es_color_oscuro(*color):
            lighten_value = 40
        else:
            lighten_value = -10

        # Adjust the L channel
        l_channel = np.clip(l_channel + lighten_value * blurred_mask, 0, 255)

        # Apply the new color to the masked region with blending
        a_channel = (1 - blurred_mask) * a_channel + blurred_mask * color_lab[1]
        b_channel = (1 - blurred_mask) * b_channel + blurred_mask * color_lab[2]

        # Merge the channels back
        updated_lab = cv2.merge([l_channel.astype(np.uint8), a_channel.astype(np.uint8), b_channel.astype(np.uint8)])
        updated_rgb = cv2.cvtColor(updated_lab, cv2.COLOR_LAB2RGB)

        # Save the result
        plt.imsave(output_path, updated_rgb)
        print(f"La imagen con el cabello teñido se ha guardado en {output_path}")

        # Display the result
        plt.figure(figsize=(10, 10))
        plt.imshow(updated_rgb)
        plt.axis('off')


    def refine_hair_mask(self, initial_mask, color_threshold=100):
        """
        Refine the hair mask based on the color information.
        z
        Args:
            initial_mask (dict): The initial mask generated by SAM.
            color_threshold (int): The threshold for color similarity.
        Returns:
            np.ndarray: The refined mask.
        """
        if self.image is None:
            raise ValueError("No image loaded. Please load an image first.")
        # Calculate the mean color of the hair region in the initial mask
        hair_pixels = self.image[initial_mask]
        mean_color = np.mean(hair_pixels, axis=0)

        # Create a new mask based on color similarity
        refined_mask = np.zeros_like(initial_mask, dtype=np.uint8)
        for i in range(self.image.shape[0]):
            for j in range(self.image.shape[1]):
                pixel_color = self.image[i, j]
                if np.linalg.norm(pixel_color - mean_color) < color_threshold and initial_mask[i][j]:
                    refined_mask[i, j] = 1
        return refined_mask
